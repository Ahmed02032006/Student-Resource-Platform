...
const AdminDashboardLayout = () => {
  ...
  const tabs = [
    ...
    },
    {
      ...
      icon: UserCheck,
      label: "Approval & Registration",
      path: "/admin/approvals"
    },
    {
      name: "Course and Assets",
      icon: BookOpen,
      label: "Course and Assets",
      path: "/admin/courses"
    },
    {
  ...
  const getHeaderDetails = () => {
    if (location.pathname === '/admin/approvals') {
      return {
        heading: "Approval & Registration Workspace",
        subHeading: "Review student account registrations and course access requests"
      };
    }
    if (location.pathname === '/admin/courses') {
      return {
        heading: "Course & Asset Registry",
        subHeading: "Create academic course entries and publish digital study resources"
      };
    }
    if (location.pathname === '/admin/activity') {
      return {
        heading: "Audit Activity Logs",
        subHeading: "Real-time track of user actions, enrollment events, and administrative decisions"
      };
    }
    return {
      heading: "Admin Dashboard",
      subHeading: "Overview of system accounts, active courses, and access metrics"
    };
  };
    ...
    <div className="h-screen flex flex-col overflow-hidden bg-slate-50">
      <div className="flex flex-1 min-h-0">
            ...
            <div className="py-[9px] border-b border-slate-200 w-full bg-gradient-to-r from-slate-50 to-white">
              <div className="flex items-center justify-center">
                {sidebarOpen ? (
                  ...
                  >
                    <p className="text-2xl orbitron text-slate-900 font-bold tracking-wider">ADMIN PANEL</p>
                  </div>
                ) : (
                  <div 
                    onClick={() => navigate("/admin/dashboard")}
                  ...
                  >
                    <span className="orbitron text-sm">A</span>
                  </div>
                )}
              </div>
            </div>
                        ...
                        <div className="ml-4 mt-1 mb-2 space-y-1">
                          {subItems.map((subItem) => (
                            <button
                              ...
                              className={`w-full text-left py-2 px-4 text-sm rounded transition-all ${
                ...
                <div className="flex items-center gap-3 p-2 rounded-xl bg-gradient-to-r from-blue-50 to-slate-50 border border-blue-100">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10.5px] text-slate-400 truncate" title={user?.email}>
                      {formatEmail(user?.email || 'admin@acadex.edu')}
                    </p>
                  </div>
                  <button
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          <HeaderComponent 
            heading={headerDetails.heading} 
            subHeading={headerDetails.subHeading} 
            role="superAdmin" 
          />
